...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className="hidden md:flex h-full flex-col overflow-y-auto bg-white shadow-sm borger-r">
        {routes.map((route) => {
          return (
            <Link
              key={route.href}
        ...
        {routes.map((route) => {
          return (
            <Link
              ...
              href={route.href}
              className={cn(
                "flex flex-col items-center justify-center text-slate-500 text-xs font-medium transition- py-1 flex-1",
                pathname === route.href
                  ? "text-blue-700 bg-blue-100/50 hover:bg-blue-100 hover:text-blue-700"
                  : ""
              )}
